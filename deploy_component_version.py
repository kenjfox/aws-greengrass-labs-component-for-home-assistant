...
def get_newest_component_version(component_name):
    ...
    component_arn = "arn:aws:greengrass:{}:aws:components:{}".format(
        gdk_config.region(), component_name
    )
    ...
    try:
        ...
        print(
            "Failed to get component versions for {}\nException: {}".format(
                component_name, e
            )
        )
        sys.exit(1)
...
def get_deployment():
    ...
    thing_arn = "arn:aws:iot:{}:{}:thing/{}".format(
        gdk_config.region(), ACCOUNT, args.coreDeviceThingName
    )

    print(
        "Searching for existing single Thing deployment for {}".format(
            args.coreDeviceThingName
        )
    )
    ...
    try:
        ...
        response = greengrassv2_client.list_deployments(
            targetArn=thing_arn, historyFilter="LATEST_ONLY", maxResults=1
        )
    except Exception as e:
    ...
    try:
        ...
        if "deploymentName" in response:
            print(
                'Found existing named deployment "{}"'.format(
...
def update_deployment(deployment):
    ...
    if COMPONENT_DOCKER_APPLICATION_MANAGER not in deployment["components"]:
        ...
        print(
            "Adding {} {} to the deployment".format(
                COMPONENT_DOCKER_APPLICATION_MANAGER, version
            )
        )
        deployment["components"].update(
            {COMPONENT_DOCKER_APPLICATION_MANAGER: {"componentVersion": version}}
        )
    ...
    if COMPONENT_SECRET_MANAGER not in deployment["components"]:
        ...
        print(
            "Adding {} {} to the deployment".format(COMPONENT_SECRET_MANAGER, version)
        )
        ...
        merge_str = deployment["components"][COMPONENT_SECRET_MANAGER][
            "configurationUpdate"
        ]["merge"]
        ...
        if secret_value["ARN"] not in merge_str:
            print(
                "Adding secret {} to Secret Manager configuration".format(
                    secret_value["ARN"]
                )
            )
    ...
    deployment["components"].update(
        {
            COMPONENT_SECRET_MANAGER: {
                ...
                "configurationUpdate": {
                    "merge": '{"cloudSecrets":' + json.dumps(cloud_secrets) + "}"
                },
            }
        }
    )
    ...
    deployment["components"].update(
        {gdk_config.name(): {"componentVersion": args.version}}
    )
...
def wait_for_deployment_to_finish(deploy_id):
    ...
    if deployment_status == "COMPLETED":
        print(
            "Deployment completed successfully in {:.1f} seconds".format(
                time.time() - snapshot
            )
        )
        print(response)
...
parser = argparse.ArgumentParser(
    description="Deploy a version of the {} component".format(gdk_config.name())
)
parser.add_argument(
    "version", help="Version of the component to be deployed (Example: 1.0.0)"
)
...
print(
    "Attempting deployment of version {} to core device {}".format(
        args.version, args.coreDeviceThingName
    )
)
...
print(
    "Deployment {} successfully created. Waiting for completion ...".format(
        new_deployment_id
    )
)
